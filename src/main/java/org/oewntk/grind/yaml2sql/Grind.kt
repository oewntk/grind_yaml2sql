...
object Grind {
    ...
    fun main(args: Array<String>) {
        ...
        val model = Factory(inDir, inDir2, inverses = inverses, throws = !doNotThrow, verbose = verbose).get()
        progress("after model is supplied", startTime, verbose = verbose)

        // Consume model
        progress("before model is consumed", startTime, verbose = verbose)
        ModelConsumer(outDir, compatSchema = compat, skipInverses = skipInverses, verbose = verbose).accept(model!!)
        progress("after model is consumed", startTime, verbose = verbose)

        // End
        progress("total", startTime, verbose = verbose)
    }
}
